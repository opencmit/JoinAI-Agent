...
from langgraph_agent.graph.graph import AgentGraph  # 导入核心功能
...
async def test_llm_parameter_extraction():
    ...
    test_mcp_tools = [
        ...
        # }
        {
            "desc": "输入两个文本，输出相似度分值（0~1），用于公文查重、内容去重等场景。",
            "name": "text_common_sim",
            "type": "normal",
            "tool_id": "M0060100012@@text_common_sim",
            "user_id": "b5b8638c-2345-4b8d-9b9b-e67aac6e4139",
            "arguments": {
                "type": "object",
                "required": [
                    "req_id",
            ...
            "mcp_server_url": "https://117.134.46.9:31881/mcp/9cf66d2af65641859cc00ad5602dc0adoaho8c/sse"
        }
    ]
    
    # 创建包含MCP工具的状态
    initial_state = AgentState(
        messages=[HumanMessage(content="今天天气很糟糕。今天下大暴雨。请调用工具查询这两句话相似度有多高？")],
...
async def test_knowledge_base_mcp_tool():
    ...
    try:
        # 测试知识库工具转换
        mcp_tools = agent.mcp_tools_manager.get_mcp_tools_from_state(initial_state, mock_llm)
...
async def test_agent_with_files_tool():
    ...
    initial_state = AgentState(
        ...
        messages=[
            ...
            HumanMessage(content="深圳今天的天气真糟糕。深圳今天下大暴雨。这两句话有什么多少相似度？")
        ],
        mcp_tools=test_mcp_tools,
        model="Qwen/Qwen3-235B-A22B"
    )
...
if __name__ == "__main__":
    ...
    # 运行所有测试
    asyncio.run(test_agent_with_files_tool())
