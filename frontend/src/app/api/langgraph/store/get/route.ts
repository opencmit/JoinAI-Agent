...
export async function POST(req: NextRequest) {
    console.log('[LangGraph获取一个item的接口] req.url', req.url);

    // 获取请求体内容
    const payload: { namespace: string[], key: string } = await req.json();

    try {
        // 调用LangGraph SDK获取一个item
        const result = await client.store.getItem(payload.namespace, payload.key);
