 ...
 * LangGraph会话列表查询接口
 * 
 * 该接口用于查询LangGraph会话列表，基于LangChain LangGraph SDK，
 * 支持以下功能：
 * 1. 搜索和获取LangGraph会话列表
 * 2. 支持Mock数据模式
 * 3. 支持搜索条件过滤
 * 4. 返回会话列表数据
 * 
...
import { SearchThread } from "@/types/langgraph";
...
/**
 * 获取LangGraph会话列表的POST接口
 * 
 * 功能说明：
 * 1. 接收POST请求，搜索LangGraph会话列表
 * 3. 支持搜索条件过滤会话
 * 4. 返回会话列表数据
 * 
 * @param req - Next.js请求对象
 * @returns NextResponse - 包含会话列表数据的响应
 * 
 * 请求参数：
 * - 根据SearchThread类型定义的搜索条件
 * 
 * 响应格式：
 * - success: 请求是否成功
 * - message: 响应消息
...
export async function POST(req: NextRequest) {
    console.log('[获取LangGraph会话列表接口] req.url', req.url);

    // 获取请求体内容
    const payload: SearchThread = await req.json();
